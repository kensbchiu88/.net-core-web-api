...
{
    public class SmtUnitProcessCheckByFixturesnCommand : IMesCommand
    {
        public string CommandName { get; } = "SMT_UNIT_PROCESS_CHECK_BY_FIXTURESN";

        private readonly IMesService _equipmentService;

        public SmtUnitProcessCheckByFixturesnCommand(IMesService equipmentService)
        {
        ...
        public async Task<MesCommandResponse> Execute(MesCommandRequest input)
        {
            ...
            try
            {
                string mesReturn = await _equipmentService.SMT_UNIT_PROCESS_CHECK_BYFIXTURE(inputModel.OPRequestInfo!.FixtureSn!, inputModel.SectionCode!, inputModel.StationCode!.ToString()!);
